<script lang="ts">
	...
	import { obtainDate, seededShuffle } from '$lib/utils';
	...
	async function handleNo() {
		...
		try {
			...
			responseData._timestamp = obtainDate();
	...
	async function handleSave() {
		...
		for (const w of activeWidgets) {
			if (w.required && !widgetValues[w.id]?.trim()) {
		...
		try {
			...
			for (const w of activeWidgets) {
				if (w.type === 'audio-recording') {
					...
					responseData[`${w.id}_start`] = start || null;
					responseData[`${w.id}_end`] = end || null;
				} else {
					responseData[w.id] = widgetValues[w.id] || null;
				}
			}
			responseData._timestamp = obtainDate();
...
<svelte:head>
	<title>{config ? i18n.localized(config.metadata.title) : 'Survey'}</title>
</svelte:head>
			...
			<div class="flex gap-3">
				{#if nextPhase}
					<button
						...
						onclick={() => { goto(`/e/${slug}/${nextPhase.slug}`); }}
					>
