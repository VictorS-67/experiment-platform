...
## Real-world testing fixes — slow-network + custom scrubber + tutorial popover hardening (2026-05-07)

A round of in-the-wild testing surfaced six bugs (notes/bug_notes.md): tutorial stuck on slow internet, admin participant detail too slow + unreadable, video slider lag/snap-back/freeze. Followed by a custom-scrubber rewrite and a tutorial popover-positioning workaround. Two commits' worth of work.

### Slow-network + admin perf (commit `626df6f`)

- **Tutorial → first phase navigation never completed on Slow 3G.** `goto()` was unawaited; the destination's load function (signed URL minting + chunk routing) stalled silently and the participant saw no spinner. Fixed: `await goto`, loading curtain, 10 s timeout that surfaces a "Try again" button. ([+page.svelte](src/routes/e/[slug]/tutorial/+page.svelte))
- **Tutorial intro modal took ~35 s to appear on Slow 3G.** Streamed `signedUrls` and `firstChunkUrl` from the page-load as un-awaited promises so SvelteKit ships the page synchronously and resolves them in the background. Also passes an `idFilter` to `signStimuliUrls` so the tutorial signs only the sample stimulus, not every stimulus in the experiment. Net: ~3 s to modal vs ~35 s. ([+page.server.ts](src/routes/e/[slug]/tutorial/+page.server.ts))
- **Tutorial intro modal didn't render in SSR.** The component derived `tutorial` from a client-only store populated in a `$effect`; gated `{#if tutorial}` was therefore omitted from the server-rendered HTML. Switched to deriving from `data.experiment.config` (which IS in SSR) so the modal markup ships with the initial response.
- **Admin participant detail page took 1–2 minutes for a participant with ~250 responses.** Multiple causes: (a) duplicate `experiments` SELECT (layout already loaded it); (b) duplicate `responses` SELECT in `getParticipantSessionTimings`; (c) for an audio-collecting experiment, every saved audio rendered an `<audio controls>` element which fired its own metadata-fetch in parallel; (d) a flat dump of 250 cards with sentinel `_chunk` / `_timestamp` rows visible in each. Fixed: `parent()` reuses the layout's experiment, `computeSessionTimings(config, responses)` is a pure function called in-process, `<audio preload="none">`, restructured the responses panel as **chunk → block → stimulus** collapsibles (single source of truth: stimulus prompt + widget labels resolved from config). ([+page.server.ts](src/routes/admin/experiments/[id]/participants/[participantId]/+page.server.ts), [+page.svelte](src/routes/admin/experiments/[id]/participants/[participantId]/+page.svelte), [admin.ts](src/lib/server/admin.ts))
- **Video slider lag, snap-back-to-zero on short clips, and "freeze until end" after multiple adjustments.** Switched [VideoPlayer](src/lib/components/stimuli/VideoPlayer.svelte) `preload="metadata"` → `preload="auto"`. Hardened the [replay controller](src/lib/utils/replay.ts) with a `seeking` listener that self-clears the segment listener if the user manually scrubs (preventive — relevant once review/replay configs are added; not the cause of the user's freeze, that was preload-driven), and added a `replayController.cleanup()` call in `resetForCurrentItem` so segment listeners can't survive a stimulus change.
- **Review highlight ring was a 600 ms one-shot pulse fading to invisible.** Rewrote `.stimulus-highlight` in [app.css](src/app.css) as a sustained thick yellow ring + glow with no transition, keyed on the `highlightActive` state. Updated `replayFullWithHighlight` to drive the highlight from `requestAnimationFrame` (~60 Hz) so the ring stays visually synced with `currentTime` instead of flickering on/off at `timeupdate`'s ~4 Hz cadence.
- **Sync script choked on `pending_invites_invited_by_fkey`.** `pending_invites.invited_by` references remote `auth.users(id)`; those UUIDs don't exist locally. Set `invited_by: null` on insert (the FK is nullable for exactly this case). ([scripts/sync-remote-to-local.js](scripts/sync-remote-to-local.js))

### Custom video scrubber + tutorial popover hardening (uncommitted at the time of writing)

- **Native Chrome `<video controls>` slider doesn't seek live during drag.** The video frame doesn't follow the slider thumb; only updates on release. Replaced with a custom [MediaScrubber.svelte](src/lib/components/stimuli/MediaScrubber.svelte) that owns play/pause, time readout, and a range slider that calls `media.fastSeek(t)` (Chrome/Safari) or `media.currentTime = t` (Firefox) on every input event. **Coalesced seeks**: only one `seeked` is in flight at a time — fast scrubs no longer queue up requests the decoder can't drain. Adds a `markers?: ScrubberMarker[]` prop for visual start/end indicators on the timeline; phase page + tutorial page derive markers from any active timestamp-range widget value, ReviewItemDisplay derives them from the saved range.
- **Progress fill overlapping the thumb on the new scrubber.** Native range thumb anchors its right edge to track end at max, not its centre. A plain `width: {percent}%` progress fill therefore overshoots the thumb's centre by half a thumb-width. Compensated with a calc using `--p` (percent) and `--thumb-w` (CSS variable for thumb size — single source of truth across the calc and the pseudo-element).
- **Tutorial popover for `#stimulus-player` steps landed on the scrubber on narrow viewports.** `#stimulus-player` wraps the (passive) video frame AND the (interactive) scrubber. On narrow viewports Driver.js can't fit the popover on any side and falls back to a viewport-centred placement (signalled via the `driver-popover-arrow-none` class) that lands on the scrubber. Detect that fallback in `onHighlighted` (`maybeOverlayPopoverOnPlayerTop`) and pin the popover to the top of the viewport so it overlays only the (passive) video frame. The `pickSide` rule (extracted to [tutorial-placement.ts](src/lib/utils/tutorial-placement.ts)) handles the cleaner `bottom → top` conversion up front. A scoped `:global(.driver-popover:not([data-tutorial-ready='true']))` opacity-0 rule plus `animate: false` in the driver config prevents the user from seeing the popover at the wrong (initial) position before our reposition runs — Driver.js's CSS animation otherwise overrides inline `opacity:0` while it plays.

### Tests added

- **[src/lib/utils/tutorial-placement.test.ts](src/lib/utils/tutorial-placement.test.ts)** — 7 cases pinning down `pickSide` and `mapSide`. The `bottom → top` rule for `#stimulus-player` is the regression-bait: if it gets reverted, this fails fast.
- **[src/lib/server/admin.test.ts](src/lib/server/admin.test.ts)** — 7 cases for `computeSessionTimings` (was integration-only before the refactor): empty input, basic aggregation, anchor routing via `_chunk` sentinel, unknown-chunk fallback, orphan stimuli, sort order, label preservation.
- **[src/lib/utils/replay.test.ts](src/lib/utils/replay.test.ts)** — added a `_simulateSeeking` mock helper and one new test for the seeking-cancel behaviour added to `replaySegment`.
- **[tests/e2e/participant/tutorial-narrow-viewport.spec.ts](tests/e2e/participant/tutorial-narrow-viewport.spec.ts)** — defensive Playwright spec at 1024×720 that builds an inline tutorial config targeting `#stimulus-player`, inflates the player to a 60vh-equivalent size, asserts the popover lands at viewport top (`y ≤ 32 px`) instead of viewport-centred. Catches a future Driver.js renaming `arrow-none` because that's the class our overlay detection sniffs.

Total: 12 new unit tests (193 → 205 passing), 1 new E2E spec.

### Honest notes on what's still load-bearing

The tutorial popover overlay leans on three Driver.js implementation details: (a) the `driver-popover-arrow-none` class as a "no clean side fits" signal, (b) `onPopoverRender`/`onHighlighted` callback timing, (c) `animate: false` to disable the fade. If Driver.js renames any of those, the runtime override silently breaks — the E2E spec above is the canary. Documented in CLAUDE.md gotchas.
