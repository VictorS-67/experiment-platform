...
- **Email-link path — fresh invitee** (Supabase `auth.users` row does NOT yet exist for this email): `inviteCollaboratorByEmail` calls `auth.admin.inviteUserByEmail`, which creates the auth row and emails an invite link. Clicking it → Supabase's hosted invite handler signs them in via OTP → bounces to `/admin/login?claim=...#access_token=...&type=invite` → login page short-circuits to `/admin/reset-password?claim=...#fragment` → they set a password → `/admin/login?claim=...&reset=success` → sign in → `claimInvitesForUser` runs and the collaborator row is created.
- **Email-link path — already-registered invitee** (auth row exists, e.g. from a prior unclaimed invite): `inviteUserByEmail` would reject with "already registered". `sendInviteOrRecoveryEmail` falls back to `auth.resetPasswordForEmail` with `redirectTo` pointing at `/admin/reset-password?claim=...`. Clicking the recovery email lands them DIRECTLY on the password-set page (Supabase puts `type=recovery` in the fragment, which the page already handles). Same end-state: `/admin/login?claim=...&reset=success` → sign in → invite claims.
- **Resend** uses the same branching: `resendPendingInvite` looks up the auth user and calls `sendInviteOrRecoveryEmail` with the right `isExistingUser` flag — so resending a stuck invite always sends a working email regardless of whether the user has been created yet.
- **Manual claim-URL path (rare)**: invitee was given just the claim URL (no email) — they land on `/admin/login?claim=...` with no fragment, see the "you've been invited" banner that points them at **Forgot password?**, request a reset email through the platform's own forgot-password endpoint, set a password through the same `/admin/reset-password` page, then sign in.
